...
`Hospital` وكل صنف بتشير له بيطبّقوا `Serializable`. خيارَي **Save Data to
File** / **Load Data from File** بقائمة `Main` تبع الكونسول بيسلسلوا
ويفكّوا تسلسل كامل شجرة كائن `Hospital` لملف `medisys-data.dat` بتسلسل
كائنات جافا القياسي — بدون مكتبات خارجية أو قاعدة بيانات مطلوبة.
